...
def read_keypoints2(json_input, size, transform_A, random_drop_prob=0, remove_face_labels=False):
    with open(json_input) as f:
        keypoint_dicts = json.loads(f.read())

    edge_lists = define_edge_lists2()
    w, h = size    
    pose_pts = np.array(keypoint_dicts).reshape(18, 3)
    pts = extract_valid_keypoints2(pose_pts, edge_lists)         
    pose_img, dist_tensor = connect_keypoints2(pts, edge_lists, size, random_drop_prob, remove_face_labels, transform_A)
    return pose_img, dist_tensor

def extract_valid_keypoints2(pts, edge_lists):
...
def connect_keypoints2(pts, edge_lists, size, random_drop_prob, remove_face_labels, transform_A):
    pose_pts = pts
    w, h = size
    ...
    for i, edge in enumerate(pose_edge_list):
        ...
        if (np.random.rand() > random_drop_prob) and (0 not in x):
            ...
            drawEdge(output_edges, curve_x, curve_y, bw=2, color=pose_color_list[i], draw_end_points=True)
            drawEdge(im_edge, curve_x, curve_y)
        im_dist = cv2.distanceTransform(255-im_edge, cv2.DIST_L1, 3)    
        im_dist = np.clip((im_dist / 2), 0, 255).astype(np.uint8)
        im_dist = Image.fromarray(im_dist)
        tensor_cropped = transform_A(im_dist)                    
        dist_tensor = tensor_cropped if e == 1 else torch.cat((dist_tensor, tensor_cropped), 0)
        e += 1
    return Image.fromarray(output_edges), dist_tensor
...
def define_edge_lists2():
    ...
    pose_color_list = [
        ...
        [  0,153, 51], [  0,153,102], [  0,153,153], [  0,102,153], [  0,51,153], [  0,0,153],
