...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        ############## Forward Pass ######################
        losses, generated1, mask, generated = model(
        ...
        loss_dict['G_confidence'] *= opt.G_confidence_mul
        loss_dict['FG_GAN'] *= opt.FG_GAN_mul
        ...
        loss_FG = loss_dict['FG_GAN'] + loss_dict.get('FG_GAN_Feat',0) + loss_dict.get('FG_VGG',0)
        if (total_steps > opt.use_style_iter):
            loss_D = loss_D + loss_dict['D_fake_final']
            loss_G = loss_G + loss_dict['G_confidence'] + loss_FG
        ...
        if (total_steps > opt.use_style_iter):
            model.module.optimizer_FG.zero_grad()
            loss_FG.backward()
            model.module.optimizer_FG.step()
        ...
        if total_steps % opt.val_freq == 0:
            ...
            for j in range(opt.val_n_everytime):
                ...
                '''
                data_test = {}
                for k, v in data_test_.items():
                    data_test[k] = v.unsqueeze(0)#np.expand_dims(v, axis=0)
                '''
                ############## Forward Pass ######################
                losses, generated1_test, mask_test, generated_test = model(
                        ...
                        Variable(data_test['D']), Variable(data_test['D2']), infer=(j == opt.val_n_everytime - 1))
                losses = [ torch.mean(x) if not isinstance(x, int) else x for x in losses ]
            ...
            loss_dict['loss_G'] = loss_dict['G_GAN'] + loss_dict['G_GAN_style'] * use_style + loss_dict.get('G_GAN_Feat',0) + loss_dict.get('G_VGG',0)
            loss_dict['loss_FG'] = loss_dict['FG_GAN'] + loss_dict.get('FG_GAN_Feat',0) + loss_dict.get('FG_VGG',0)
            if (total_steps > opt.use_style_iter):
                loss_dict['loss_D'] = loss_dict['loss_D'] + loss_dict['D_fake_final']
                loss_dict['loss_G'] = loss_dict['loss_G'] + loss_dict['G_confidence'] + loss_dict['loss_FG']

            for k, v in loss_dict.items():
                train_writer.add_scalar('val/' + k, v, total_steps)

            train_writer.add_image('val/input_label', util.tensor2label(data_test['A'][0], 0), total_steps)
            train_writer.add_image('val/real_image', util.tensor2im2(data_test['A2'][0]), total_steps)
            train_writer.add_image('val/synthesized_image_1', util.tensor2im2(generated1_test.data[0]), total_steps)
            train_writer.add_image('val/mask', util.tensor2im2(mask_test.data[0]), total_steps)
            train_writer.add_image('val/synthesized_image', util.tensor2im2(generated_test.data[0]), total_steps)
            train_writer.add_image('val/B', util.tensor2label(data_test['B'][0], 0), total_steps)
        ...
        if save_fake:
            visuals = OrderedDict([('input_label', util.tensor2label(data['A'][0], 0)),
                                   ...
                                   ('synthesized_image_1', util.tensor2im(generated1.data[0])),
                                   ('mask', util.tensor2im(mask.data[0])),
            ...
            train_writer.add_image('train/input_label', util.tensor2label(data['A'][0], 0), total_steps)
            train_writer.add_image('train/real_image', util.tensor2im2(data['A2'][0]), total_steps)
            train_writer.add_image('train/synthesized_image_1', util.tensor2im2(generated1.data[0]), total_steps)
            train_writer.add_image('train/mask', util.tensor2im2(mask.data[0]), total_steps)
            train_writer.add_image('train/synthesized_image', util.tensor2im2(generated.data[0]), total_steps)
            train_writer.add_image('train/B', util.tensor2label(data['B'][0], 0), total_steps)
