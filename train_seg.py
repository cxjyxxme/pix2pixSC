...
data_loader_test = CreateConDataLoader(opt_test)
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        if total_steps % opt.val_freq == 0:
            ...
            for j in range(len(opt.train_val_list)):
                ...
                ans.append([util.tensor2im2(data_test['A'][0]), 
                        util.tensor2im2(data_test['A2'][0]),
                        util.tensor2im2(generated_test.data[0]), 
                        util.tensor2im2(data_test['B'][0]), 
            ...
            for j in range(opt.val_n_everytime):
                ...
                losses, generated_test = model(
                        ...
                        Variable(data_test['B']), Variable(data_test['B2']),
                        Variable(data_test['C']), Variable(data_test['C2']),
                        Variable(data_test['D']), Variable(data_test['D2']), same_style=Variable(data_test['same_style']), infer=(j == opt.val_n_everytime - 1), current_iter=total_steps)
                losses = [ torch.mean(x) if not isinstance(x, int) else x for x in losses ]
                loss_dict_ = dict(zip(model.module.loss_names, losses))
                for k, v in loss_dict_.items():
                    if not isinstance(v, int):
            ...
            train_writer.add_image('val/input_label', util.tensor2label2(data_test['A'][0], 0), total_steps)
            ...
            train_writer.add_image('val/B', util.tensor2label2(data_test['B'][0], 0), total_steps)
        ...
        if save_fake:
            ...
            train_writer.add_image('train/input_label', util.tensor2label2(data['A'][0], 0), total_steps)
            ...
            train_writer.add_image('train/B', util.tensor2label2(data['B'][0], 0), total_steps)
