...
def get_features(inst, feat):
    ...
    for i in np.unique(inst):
        ...
        for k in range(feat_num):
            val[k] = feat[idx[0] + k, idx[1], idx[2]].data[0]    
...
for k in range(opt.n_clusters):
    # create website
    web_dir = os.path.join(opt.results_dir, opt.name, '%s_%s_%d' % (opt.phase, opt.which_epoch, k))
    ...
    for i, data in enumerate(dataset):
        ...
        if opt.engine:
            ...
            generated = model.inference_conditioned(data['label'], data['inst'], con, k)
        else:
